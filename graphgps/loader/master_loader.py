...
def load_dataset_master(format, name, dataset_dir):
    ...
    if format.startswith('PyG-'):
        ...
        if pyg_dataset_id == 'GNNBenchmarkDataset':
            # dataset_dir = '/mnt/share/yxd/Graph-Mamba-main/datasets/GNNBenchmarkDataset'
        ...
        elif pyg_dataset_id == 'ABIDE':
            dataset = CustomGNNBenchmarkDataset('')
            dataset = preformat_ABIDE(dataset)
...
def preformat_ABIDE(dataset):
    train_indices = torch.load(os.path.join(dataset.processed_dir, 'train_index.pt'))
    val_indices = torch.load(os.path.join(dataset.processed_dir, 'val_index.pt'))
    test_indices = torch.load(os.path.join(dataset.processed_dir, 'test_index.pt'))
    
    train_dataset = dataset#CustomGNNBenchmarkDataset(dataset_dir, transform=None, pre_transform=None, pre_filter=None)
    train_dataset = train_dataset[train_indices]
    val_dataset = dataset#CustomGNNBenchmarkDataset(dataset_dir, transform=None, pre_transform=None, pre_filter=None)
    val_dataset = val_dataset[val_indices]
    test_dataset = dataset#CustomGNNBenchmarkDataset(dataset_dir, transform=None, pre_transform=None, pre_filter=None)
    test_dataset = test_dataset[test_indices]

    combined_dataset = join_dataset_splits([train_dataset, val_dataset, test_dataset])
...
# from torch_geometric.data.download import download_url, extract_zip
# class CustomGNNBenchmarkDataset(InMemoryDataset):
...
#         return ['data.pt', 'train_index.pt', 'val_index.pt', 'test_index.pt']
...
#     def process(self):
#         data_list = []
#         loadData = np.load(os.path.join(self.raw_dir, self.raw_file_names[0]), allow_pickle=True).item()
#         corr_matrices = loadData["corr"]
#         labels = loadData["label"]
...
#             corr_matrix = zero_diagonal(corr_matrix)

#             edge_index = np.array(np.where(np.abs(corr_matrix) >= threshold_value))
#             edge_attr = corr_matrix[edge_index[0], edge_index[1]]
...
#         torch.save(train_indices, self.processed_paths[1])
#         torch.save(val_indices, self.processed_paths[2])
#         torch.save(test_indices, self.processed_paths[3])
class CustomGNNBenchmarkDataset(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
        super().__init__(root, transform, pre_transform, pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self):
        ...
        for i in range(len(corr_matrices)):
            corr_matrix = corr_matrices[i]
            corr_matrix = zero_diagonal(corr_matrix)
            num_elements = corr_matrix.size
            top_5_percent_index = int(num_elements * percent_threshold)
            threshold = np.sort(np.abs(corr_matrix).flatten())[-top_5_percent_index]

            edge_index = np.array(np.where(np.abs(corr_matrix) >= threshold))
            edge_attr = corr_matrix[edge_index[0], edge_index[1]]
        ...
        torch.save(self.collate(data_list), self.processed_paths[0])
        torch.save([i for i in range(train_size)], self.processed_paths[1])
        torch.save([i for i in range(train_size, train_size + val_size)], self.processed_paths[2])
        torch.save([i for i in range(train_size + val_size, len(data_list))], self.processed_paths[3])
############################################################################
# class CustomGNNBenchmarkDataset(InMemoryDataset):
#     def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
#         super().__init__(root, transform, pre_transform, pre_filter)
...
# # 使用示例
# root = '/mnt/share/yxd/Graph-Mamba-main/datasets/GNNBenchmarkDataset/AB'
# dataset = CustomGNNBenchmarkDataset(root)
# print(dataset[0])
